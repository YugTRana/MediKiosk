...
export default function DoctorDashboard() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 font-sans p-4 md:p-8 flex flex-col gap-6 select-none ${isDarkMode ? 'dark-theme bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      {/* Top Header */}
      <header className="bg-slate-900 text-white p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg font-bold tracking-tight">MediKiosk Physician Review Workspace</h1>
            </div>
        ...
        {/* Global Navigation & Settings Dropdown */}
        <div className="relative flex items-center gap-2">
          <LanguageToggle />
          <button
            onClick={() => setIsDarkMode(!isDarkMode)}
            className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl flex items-center justify-center border border-slate-700 transition-colors cursor-pointer"
          ...
          <button
            ...
            className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl border border-slate-700 transition-all flex items-center gap-2 cursor-pointer"
            title="Settings & Actions"
          >
            <Settings className="w-4 h-4 text-slate-400" /> Actions
          </button>

          {isSettingsMenuOpen && (
            <div className="absolute right-0 top-12 mt-1 w-64 bg-slate-800 border border-slate-700 rounded-2xl shadow-xl z-50 flex flex-col p-2 gap-1 animate-fadeIn">
              
              <div className="bg-slate-900/50 rounded-xl p-3 flex flex-col gap-2.5 mb-1 border border-slate-700/50">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold text-slate-400">Acceptance Rate</span>
                  <span className="text-sm font-black text-amber-400 font-mono">{metrics.acceptanceRate}%</span>
                </div>
                <div className="h-px bg-slate-700/50 w-full"></div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold text-slate-400">System Mode</span>
                  <span className="text-[10px] font-bold text-amber-400 bg-amber-400/10 px-1.5 py-0.5 rounded border border-amber-400/20">Sandbox</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase font-bold text-slate-400">FHIR Sync</span>
                  <span className="text-[10px] font-bold text-emerald-400 bg-emerald-400/10 px-1.5 py-0.5 rounded border border-emerald-400/20 flex items-center gap-1">
                    <Code2 className="w-3 h-3" /> R4 Connected
                  </span>
                </div>
              </div>
              ...
              <a
                href="/kiosk"
                className="w-full text-left px-3 py-2 hover:bg-slate-700/50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-200 cursor-pointer"
              >
                <User className="w-4 h-4 text-emerald-400" /> Patient Kiosk
              </a>

              <button
                onClick={() => { handleTriggerCronDemo(); setIsSettingsMenuOpen(false); }}
                className="w-full text-left px-3 py-2 hover:bg-purple-900/30 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-purple-300 cursor-pointer"
              >
                <BellRing className="w-4 h-4 text-purple-400" /> Test Cron Notifications
              </button>

              <div className="h-px bg-slate-700/50 my-1"></div>

              <button
                onClick={() => { handleLogoutStaff(); setIsSettingsMenuOpen(false); }}
                className="w-full text-left px-3 py-2 hover:bg-red-900/30 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-red-300 cursor-pointer"
              >
                <Lock className="w-4 h-4 text-red-400" /> Lock / Logout
              </button>
            </div>
          )}
        </div>
      ...
      {/* Main Grid: Left Session List (1 col) + Right Clinical Review Workspace (3 cols) */}
      <main className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-4 gap-6">
        
        {/* ========================================================================= */}
        {/* 1. COMPLETED PATIENT SESSIONS LIST (RED FLAGS AT TOP IN RED)               */}
        {/* ========================================================================= */}
        <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-3 h-fit max-h-[850px]">
          <div className="flex items-center justify-between border-b pb-2.5 border-slate-100">
            <h3 className="text-sm font-extrabold text-slate-900 flex items-center gap-1.5">
              <Users className="w-4 h-4 text-blue-600" /> Patient Queue ({sortedSessions.length})
            </h3>
            <span className="text-[10px] font-bold text-slate-400">Sorted by Priority</span>
          </div>
              ...
              sortedSessions.map((sess) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-2.5 rounded-xl text-left border transition-all cursor-pointer flex flex-col gap-1 relative ${
                      isRedFlag
                        ? isSelected
                          ? 'border-red-500 bg-red-50 text-red-950 shadow-sm ring-1 ring-red-400'
                          : 'border-red-200 bg-red-50/60 hover:bg-red-50 text-red-900'
                        : isAyush
                          ? isSelected
                          ...
                          : isSelected
                            ? 'border-blue-500 bg-blue-50 text-slate-900 shadow-sm ring-1 ring-blue-300'
        ...
        {/* ========================================================================= */}
        {/* 2. CLINICAL SUMMARY & INLINE PHYSICIAN REVIEW WORKSPACE (3 COLS)          */}
        {/* ========================================================================= */}
        <div className="lg:col-span-3 flex flex-col gap-6">
          {currentSession ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col gap-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-100">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="relative">
                    <button
                      onClick={() => setIsPatientActionsMenuOpen(!isPatientActionsMenuOpen)}
                      className="px-3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-extrabold rounded-xl border border-slate-300 flex items-center gap-1.5 cursor-pointer shadow-sm transition-all"
                    >
                      More <ChevronRight className={`w-3.5 h-3.5 transition-transform ${isPatientActionsMenuOpen ? 'rotate-90' : ''}`} />
                    </button>

                    {isPatientActionsMenuOpen && (
                      <div className="absolute right-0 top-12 mt-1 w-48 bg-white border border-slate-200 rounded-xl shadow-lg z-50 flex flex-col p-1.5 gap-1 animate-fadeIn">
                        
                        {/* Save Physician Amendments Button */}
                        <button
                          onClick={() => { handleSavePhysicianEdits(); setIsPatientActionsMenuOpen(false); }}
                          disabled={isSavingEdits}
                          className="w-full text-left px-3 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-700 cursor-pointer"
                        >
                          <ShieldCheck className={`w-3.5 h-3.5 ${saveEditFeedback ? 'text-emerald-600' : 'text-slate-500'}`} />
                          {isSavingEdits ? 'Saving...' : saveEditFeedback ? 'Saved ✓' : 'Save Edits'}
                        </button>

                        <a
                          href={`tel:${currentSession.patientDetails?.mobile || ''}`}
                          onClick={() => setIsPatientActionsMenuOpen(false)}
                          className="w-full text-left px-3 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-700 cursor-pointer"
                        >
                          <Phone className="w-3.5 h-3.5 text-slate-500" /> Direct Dial
                        </a>

                        <div className="h-px bg-slate-100 my-0.5"></div>

                        <button
                          onClick={() => { setDossierLang(prev => prev === 'en' ? 'hi' : 'en'); setIsPatientActionsMenuOpen(false); }}
                          className="w-full text-left px-3 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-700 cursor-pointer"
                        >
                          <Languages className="w-3.5 h-3.5 text-blue-600" />
                          {dossierLang === 'en' ? 'Translate to Hindi' : 'Translate to English'}
                        </button>

                        <button
                          onClick={() => { setShowFhirModal(true); setIsPatientActionsMenuOpen(false); }}
                          className="w-full text-left px-3 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-700 cursor-pointer"
                        >
                          <Code2 className="w-3.5 h-3.5 text-purple-600" /> View HL7 FHIR
                        </button>

                        <button
                          onClick={() => { handleAcceptAllSections(); setIsPatientActionsMenuOpen(false); }}
                          className="w-full text-left px-3 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-700 cursor-pointer"
                        >
                          <CheckCheck className="w-3.5 h-3.5 text-emerald-600" /> Accept All Edits
                        </button>

                        <button
                          onClick={() => { handleCopyNote(); setIsPatientActionsMenuOpen(false); }}
                          className="w-full text-left px-3 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors text-slate-700 cursor-pointer"
                        >
                          <Copy className="w-3.5 h-3.5 text-slate-500" /> {copyFeedback ? 'Copied to Clipboard!' : 'Copy EHR Note'}
                        </button>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap justify-between sm:justify-around bg-slate-50 p-2.5 rounded-xl border border-slate-200 shadow-sm gap-2">
                <div className="text-center px-2">
                  <span className="text-[9px] font-bold text-slate-400 uppercase block tracking-wider">Blood Pressure</span>
                  <strong className="text-sm text-slate-800 font-mono">{currentSession?.vitals?.bloodPressure || '--/--'} <span className="text-[9px] text-slate-500 font-normal">mmHg</span></strong>
                </div>
                <div className="w-px bg-slate-200 hidden sm:block"></div>
                <div className="text-center px-2">
                  <span className="text-[9px] font-bold text-slate-400 uppercase block tracking-wider">Heart Rate</span>
                  <strong className="text-sm text-slate-800 font-mono">{currentSession?.vitals?.heartRate || '--'} <span className="text-[9px] text-slate-500 font-normal">bpm</span></strong>
                </div>
                <div className="w-px bg-slate-200 hidden sm:block"></div>
                <div className="text-center px-2">
                  <span className="text-[9px] font-bold text-slate-400 uppercase block tracking-wider">SpO2</span>
                  <strong className="text-sm text-slate-800 font-mono">{currentSession?.vitals?.oxygenSat ? `${currentSession.vitals.oxygenSat}%` : '--'}</strong>
                </div>
                <div className="w-px bg-slate-200 hidden sm:block"></div>
                <div className="text-center px-2">
                  <span className="text-[9px] font-bold text-slate-400 uppercase block tracking-wider">Temperature</span>
                  <strong className="text-sm text-slate-800 font-mono">{currentSession?.vitals?.temperature || '--'} <span className="text-[9px] text-slate-500 font-normal">°F</span></strong>
                </div>
              </div>

              {/* Follow-Up Scheduling Strip */}
              <div className="flex flex-col sm:flex-row gap-3 bg-blue-50/30 p-3 rounded-xl border border-blue-100 shadow-sm items-center">
                <div className="flex-1 w-full">
                  ...
                  <input 
                    ...
                    className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-1 focus:ring-blue-400"
                  />
                </div>
                <div className="flex-1 w-full">
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Illness Severity</label>
                  <select 
                ...
                <div className="sm:w-32 flex items-center justify-center">
                  <p className="text-[9px] text-slate-400 leading-tight text-center">
                    SMS/Email reminders auto-scheduled
                  </p>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={handleCallPatient}
                    className={`px-4 py-2 text-xs font-bold rounded-lg shadow-sm cursor-pointer transition-all flex items-center gap-1.5 ${
                      isCallingAudio 
                        ? 'bg-amber-500 text-slate-950 animate-bounce' 
                        : 'bg-blue-700 hover:bg-blue-800 text-white'
                    }`}
                  ...
                  <a
                    href={`tel:${currentSession.patientDetails?.mobile || ''}`}
                    className="px-4 py-2 text-xs font-bold rounded-lg shadow-sm cursor-pointer transition-all flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300"
                  ...
                  <button
                    onClick={handleFinalizeAndPushToEmr}
                    disabled={isPushingEmr}
                    className="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold rounded-lg shadow-sm cursor-pointer transition-all flex items-center gap-1.5 disabled:opacity-50"
                  >
                    <Send className={`w-3.5 h-3.5 ${isPushingEmr ? 'animate-spin' : ''}`} />
                    {isPushingEmr ? 'Pushing to EMR...' : 'Finalize & Push to EMR'}
                  </button>
                </div>
              </div>
