...
import { 
  ...
  Stethoscope, Settings, WifiOff, Wifi, ClipboardList, Moon, Sun, Eye, EyeOff
} from 'lucide-react';
...
export default function KioskPage() {
  ...
  useEffect(() => {
    ...
    // Auto-login patient if token exists
  ...
  useEffect(() => {
    ...
    } else if (showOtpStep && otpTimer === 0) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      await sendOtp(emailToVerify);
      setOtpTimer(60);
      setCanResendOtp(false);
      setAuthSuccessMessage(`OTP resent to ${emailToVerify}`);
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (otpAction === 'login') {
        ...
        narrate(
          selectedLanguage === 'हिंदी'
            ? `बधाई हो ${profile.name} जी! आपका खाता बन गया है और आप लॉगिन हो गए हैं।`
            : `Registration successful, ${profile.name}! You are now logged in.`
        );
      }
    } catch (err) {
  ...
  return (
    <div className={`min-h-screen flex flex-col p-4 md:p-8 select-none font-sans transition-colors duration-500 ${isDarkMode ? 'dark-theme bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      {/* Kiosk Header */}
      <header className="bg-slate-900 text-white p-4 rounded-2xl shadow-sm flex flex-col sm:flex-row justify-between items-center gap-3 border border-slate-800 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg font-bold tracking-tight">MediKiosk Touch Portal</h1>
            ...
            <p className="text-slate-400 text-xs font-medium mt-0.5">Multi-lingual triage intake & AYUSH assessment</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={`p-2 rounded-lg border transition-all flex items-center justify-center cursor-pointer ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700 text-amber-300 border-slate-700' : 'bg-slate-800 hover:bg-slate-700 text-slate-200 border-slate-700'}`}
                  ...
                  <form onSubmit={handleVerifyOtp} className="flex flex-col gap-4 bg-slate-50 border border-slate-200 rounded-3xl p-6 animate-fadeIn text-center">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <button
                        ...
                        className={`text-sm font-bold ${canResendOtp && !isOtpLoading ? 'text-blue-600 hover:text-blue-800' : 'text-slate-400 cursor-not-allowed'}`}
                      >
                        {canResendOtp ? 'Resend OTP' : `Resend OTP in ${otpTimer}s`}
                      </button>
                      <button
                        type="button"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full p-3.5 pl-11 pr-11 text-sm font-bold bg-white border-2 border-slate-300 rounded-xl focus:outline-none focus:border-blue-600"
                        />
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full p-3 pr-11 text-sm font-bold bg-white border-2 border-slate-300 rounded-xl focus:outline-none focus:border-blue-600"
                        />
                        <button
                          type="button"
                          onClick={() => setShowPassword(!showPassword)}
